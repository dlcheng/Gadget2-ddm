...
{  
  ...
  init_extra_potential();
#endif                                  /* -dlcheng */


  random_kick_g = gsl_rng_alloc(gsl_rng_ranlxd1);
  gsl_rng_set(random_kick_g, 2 * ThisTask + 10);	/* start-up seeds for different cpus */
                                                        /* -dlcheng */

#ifdef DECAY_DARK_MATTER
  alloc_decay_tab();
  decay_tree_construct_flag = 0;
  ln_decay_threshold = age_of_the_system() * log(2.0) * DECAY_DAUGHT_PART_NUM / MEM_INCREASE_FACTOR / DECAY_HALF_LIFE;
  decay_new_part_threshold = 1.0 - exp(-1.0 * ln_decay_threshold);
#endif
  ...
  if(ThisTask == 0)		/* read parameter file on process 0 */
    {
      ...
      id[nt++] = DOUBLE;
/*
#if defined(DECAY_DARK_MATTER) && defined(UNEQUALSOFTENINGS)
      if(All.SofteningHalo != All.SofteningHaloMaxPhys)
        {
          printf("SofteningHalo != SofteningHaloMaxPhys, ignore SofteningHaloMaxPhys value.\n");
          All.SofteningHaloMaxPhys = All.SofteningHalo;
        }
