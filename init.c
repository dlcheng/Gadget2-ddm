...
void init(void)
{
  ...
  switch (All.ICFormat)
   ...
   undecay_part_mass = All.MassTable[1];  
   system_decay_time = 0;
   All.MassTable[1] = 0.0;    /*! tell the code to write mass block for dark matter in the output */   
#endif
  All.Time = All.TimeBegin;
  All.Ti_Current = 0;
...
#ifdef DECAY_DARK_MATTER                                          /*! -dlcheng */
      P[i].decay_flag = 1;                                        /*! flag = 1, decaying particle, flag = 0, born particle */
      P[i].decay_factor = 0.0;                                    /*! how much mass have been decayed from last time split   */
      P[i].initial_daugther_mass = P[i].Mass * INITIAL_DM_FRAC;   /*! the initial mass of the daughter particles */
#ifdef UNEQUALSOFTENINGS
      P[i].soften_length = All.ForceSoftening[P[i].Type];         /*! force soften_length of the particle                          */
#endif                                                            /*! -dlcheng */
#endif

#ifdef PETER_HALO_DECAY
      P[i].peter_decay_flag = 1;                                  /*! good to decay -dlcheng */
#endif
    }

#ifdef DECAY_DARK_MATTER                                          /*! output head will contain this */
  header.num_before_decay = All.TotNumPart;
  header.soften_length_before_decay = All.SofteningTable[1];
#endif
