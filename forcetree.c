...
{ 
  ...
  update_decay_softening();                /*! update the decayed particle softening before calculation of gravity -dlcheng */
#endif
	  ...
	  if((p = suns[j]) >= 0)
	      ...
	      else		/* a particle */
		{
		  ...
		  if(pa->Type == 0)
		    ...
		    {
#ifndef DECAY_DARK_MATTER   /* -dlcheng */
		      if(All.ForceSoftening[pa->Type] > maxsoft)
			maxsoft = All.ForceSoftening[pa->Type];
#else
                      if(pa->soften_length > maxsoft)
                        maxsoft = pa->soften_length;
#endif
		    }
#endif
#endif
...
#if defined(UNEQUALSOFTENINGS) && (!defined(ADAPTIVE_GRAVSOFT_FORGAS) && !defined(DECAY_DARK_MATTER)) /* -dlcheng */
  int maxsofttype;
#endif
#ifdef UNEQUALSOFTENINGS
#if defined(ADAPTIVE_GRAVSOFT_FORGAS) || defined(DECAY_DARK_MATTER)  /* -dlcheng */
...
#ifdef DECAY_DARK_MATTER                                                          /* -dlcheng */
          h = soft;
  	  if(h < P[no].soften_length)
            h = P[no].soften_length;        
#else
	  h = All.ForceSoftening[ptype];
	  if(h < All.ForceSoftening[P[no].Type])
	    h = All.ForceSoftening[P[no].Type];
...
void force_treeevaluate_potential(int target, int mode)
{
  ...
  while(no >= 0)
      ...
      else
	{
	  ...
	  pot += mass * h_inv * wp;                  /* self potential is included here, and remove later -dlcheng */
	}
#ifdef PERIODIC
