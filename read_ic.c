...
void read_file(char *fname, int readTask, int lastTask)
{
  ...
  double mem_increase_factor = MEM_INCREASE_FACTOR;
#endif
...
#ifdef DECAY_DARK_MATTER                                                /* -dlcheng */
      All.MaxPart = All.PartAllocFactor * (1.0 + mem_increase_factor) * (All.TotNumPart / NTask);	
      All.MaxPartSph = All.PartAllocFactor * (1.0 + mem_increase_factor) * (All.TotN_gas / NTask);	
#else
...
#endif
      allocate_memory();                                   /* allocate memory for particles on each process -dlcheng */
