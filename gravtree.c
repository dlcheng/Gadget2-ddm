...
#else
	    costtotal += force_treeevaluate_shortrange(i, 0);        /* finish the local tree, and decide whether the 
	                                                              * particle should be exported to other CPUs -dlcheng
	                                                              */ 
#endif
	    for(j = 0; j < NTask; j++)
	      {
		if(Exportflag[j])
		  {
            ...
            GravDataGet[nexport].Soft = P[i].soften_length; 
#endif
#endif
