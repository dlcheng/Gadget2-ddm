...
gsl_rng *random_kick_g;
double random_kick_tab[2];

#ifdef DECAY_DARK_MATTER
unsigned int *decay_tab_in;  /*! decay number table for CPUs -dlcheng */
unsigned int *decay_tab_out;
int decay_tree_construct_flag;      /*! indicate when to reconstruct tree */
double ln_decay_threshold;
double decay_new_part_threshold;
double undecay_part_mass;         /*! the mass of particle at the begining of the simulation -dlcheng */
double system_decay_time;     /*! the physical time since the begining -dlcheng */
#endif

#ifdef EXTRA_POTENTIAL
void (*extra_potential_accel)(FLOAT *pos, FLOAT *accel);
void (*extra_potential_accel_ad)(FLOAT *pos, FLOAT *accel, double turn_on_time);
double (*extra_potential_rho)(double r);
FLOAT ex_pot_accel[3];
#endif
