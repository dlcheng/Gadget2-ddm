...
#define MAXLEN_OUTPUTLIST   5000   /*!< maxmimum number of entries in list of snapshot output times */

#define DRIFT_TABLE_LENGTH  10000  /*!< length of the lookup table used to hold the drift and kick factors */ 
...
#define  PI              3.141592653                    /*! pi          -dlcheng                    */
...
extern gsl_rng *random_kick_g; 
extern double random_kick_tab[2]; /*! used for random kick -dlcheng */

#ifdef DECAY_DARK_MATTER
extern unsigned int *decay_tab_in;      /*! decay number table for CPUs -dlcheng */
extern unsigned int *decay_tab_out;
extern int decay_tree_construct_flag;     /*! indicate when to reconstruct tree */
extern double ln_decay_threshold;
extern double decay_new_part_threshold;
extern double undecay_part_mass;         /*! the mass of particle at the begining of the simulation -dlcheng */
extern double system_decay_time;         /*! the physical time since the begining -dlcheng */
#endif
#ifdef EXTRA_POTENTIAL
extern void (*extra_potential_accel)(FLOAT *pos, FLOAT *accel);
extern void (*extra_potential_accel_ad)(FLOAT *pos, FLOAT *accel, double turn_on_time);
extern double (*extra_potential_rho)(double r);

extern  FLOAT ex_pot_accel[3];
#endif                                                        /* -dlcheng */
...
extern struct DomainNODE
{
...
#if !defined(ADAPTIVE_GRAVSOFT_FORGAS) && !defined(DECAY_DARK_MATTER)               /*-dlcheng*/
  ...
  FLOAT maxsoft;                  /*!< hold the maximum gravitational softening of particles in the 
                                       node if the ADAPTIVE_GRAVSOFT_FORGAS or DECAY_DARK_MATTER option is selected */
#endif
#endif
}
...
extern struct particle_data
{
  ...
  double l_eff;                    /*!< the length of effective free path -dlcheng*/
#endif

#ifdef DECAY_DARK_MATTER
  int decay_flag;                /*! flag = 1, decaying particle, flag = 0, born particle */
  double decay_factor;           /*! mass fraction decayed from last split   */
  double a_born;                 /*! the expansion factor at which the decayed particle is born */
  double initial_daugther_mass;  /*! the mass of the daugther particle in the initial mixure particle 
                                     = p.mass * Initial daughter fraction */
#ifdef UNEQUALSOFTENINGS
  FLOAT soften_length;            /*! softening length of the particle  
                                   *! it is the force softening length  */
#endif                            /* -dlcheng */
#endif

#ifdef PETER_HALO_DECAY
  int peter_decay_flag;          /* 0 for decayed particle, no kick -dlcheng */ 
#endif
}
...
#else
  unsigned int num_before_decay;      /*! number of dark matter particles before decay */
  double soften_length_before_decay;   /*! softening length before any decay */
  char fill[48];                       /*! fill to 256 Bytes */
#endif
}
